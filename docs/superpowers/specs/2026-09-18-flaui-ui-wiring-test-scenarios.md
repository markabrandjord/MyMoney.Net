...
| Delete one side of an unreconciled transfer | Delete either transaction of a linked transfer pair | `MyMoney.RemoveTransfer(t)` | Both sides removed | Fixture with a 2-sided transfer, neither side reconciled | Business-layer gap: `RemoveTransfer` untested |
| Delete one side of a reconciled transfer (blocked) | Delete a transfer transaction where the *other* side is reconciled | `MyMoney.RemoveTransfer` should refuse / the delete path should reject it | Delete is blocked, transaction unchanged, per the page's documented behavior | Fixture with one side marked reconciled | Business-layer gap: this specific "reconciled side blocks delete" branch has no direct test — a genuine invalid-operation case worth covering before any UI test. **Not fully verified**: the enforcement point wasn't traced to a specific line during this audit — confirmed documented behavior, not confirmed working code |
...
| Merge two transactions that are not actually duplicates (forced) | Manually invoke merge (e.g. via drag/drop per the page) on two unrelated transactions | Same `Transaction.Merge` path | Behavior when merging genuinely unrelated transactions (does it silently combine memo/category, does it warn?) — this is the closest thing to a "genuine failure/invalid input" case for this feature and isn't described as blocked anywhere | Fixture with two clearly-unrelated transactions | Business-layer gap: no validation exists to reject a "bad" merge; worth confirming this is intentional before writing a test that expects rejection |
...
| Advanced Query builder, multi-row AND/OR | Build 2+ query rows with a Conjunction of And/Or | `MyMoney.ExecuteQuery` conjunction-combining logic | Result set matches the documented AND/OR semantics | Same fixture | Business-layer gap: same as above |
| Advanced Query on a value that matches nothing | Query for a Payee/Category that doesn't exist in the data | `MyMoney.ExecuteQuery` | Empty result set, no exception | Fixture DB, deliberately-absent search value | Business-layer gap: same as above |

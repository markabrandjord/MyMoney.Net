...
| Scenario | UI action | Business-layer API | Verify | Test data | Status |
| --- | --- | --- | --- | --- | --- |
| Add a new currency row manually | `View/Currencies`, add a new row, type a valid 3-letter code and ratio | `CurrenciesView`'s grid `InsertItem` → `Currencies.AddCurrency(currency)` | New `Currency` persists; `CultureCode`/`Name` auto-populate from `Currency.GetCultureForCurrency` when left blank | Fixture DB | Business layer written — `CurrenciesTests.cs` (`AddCurrency` persistence; also `MultiCurrencyTransactions_ComputeCorrectUsdEquivalent` covering EUR/CAD/GBP/JPY/AUD amount/ratio math, Task 7's FlaUI test covers the other 5). The grid's `CultureCode`/`Name` auto-fill-on-blank UI wiring itself is still untested |

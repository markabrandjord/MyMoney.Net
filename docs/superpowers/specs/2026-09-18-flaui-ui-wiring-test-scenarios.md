...
| Rename with Auto-Rename (creates an Alias) | Same, but check **Auto-Rename** | `RenamePayeeDialog` → `Aliases.AddAlias(alias)` + `ApplyAlias` | New `Alias` row persists mapping the raw string to the chosen payee; a later transaction with that exact payee string auto-renames on import/entry | Same fixture | Business layer partially written — `ApplyAlias`/`Aliases.AddAlias` each covered individually (`PayeesAndAliasesTests.cs`); the full round-trip (create alias via AddAlias, then have it auto-apply on a later transaction) isn't tested as one scenario |
